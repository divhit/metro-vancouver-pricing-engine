  ...
  return (
    <div className="space-y-8">
      <div className="border-b-2 border-concrete-900 pb-4">
        <h1 className="text-3xl sm:text-4xl text-concrete-900 tracking-[0.08em]" style={{ fontFamily: "var(--font-display)" }}>
          PROPERTY VALUATION
        </h1>
        <p className="text-concrete-500 text-[11px] mt-1 tracking-wider">
          ML-POWERED VALUATION // COMPARATIVE MARKET ANALYSIS
        </p>
      </div>

      {/* Search Form */}
      <form onSubmit={handleSubmit} className="card-hairline p-6 space-y-5">
        <div className="flex gap-0 border-2 border-concrete-900 w-fit">
          {(["pid", "address", "coordinates"] as const).map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => setMode(m)}
              className={`px-4 py-2 text-[10px] font-bold tracking-[0.15em] transition uppercase border-r-2 border-concrete-900 last:border-r-0 ${
                mode === m ? "bg-concrete-900 text-white" : "bg-white text-concrete-500 hover:bg-concrete-100"
              }`}
            >
              {m === "pid" ? "PID" : m === "address" ? "ADDRESS" : "COORDS"}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:items-end">
          <div className="flex-1 min-w-0">
            {mode === "pid" && (
              <div className="relative" ref={pidDropdownRef}>
                <label className="block text-xs font-medium text-concrete-500 mb-1.5">Search by PID or Street Name</label>
                <div className="relative">
                  <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-concrete-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    className="w-full pl-10 pr-4 py-2.5 rounded-none border border-concrete-200 bg-white text-concrete-900 text-sm placeholder:text-concrete-300 focus:outline-none focus:border-signal focus:ring-1 focus:ring-signal/20 transition"
                  />
                </div>
                {showPidDropdown && pidSuggestions.length > 0 && (
                  <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-concrete-200 rounded-none shadow-none overflow-hidden max-h-[400px] overflow-y-auto">
                    {pidSuggestions.map((s, i) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 flex items-center justify-between gap-3 transition ${
                          i === pidHighlight ? "bg-signal-bg" : "hover:bg-concrete-50"
                        } ${i > 0 ? "border-t border-concrete-100" : ""}`}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="text-sm text-concrete-900 font-medium truncate">{s.address}</div>
                          <div className="text-xs text-concrete-400 mt-0.5 flex items-center gap-2">
                        ...
                        <div className="text-right shrink-0">
                          <div className="text-sm font-medium text-concrete-700">{formatCurrency(s.assessed_value)}</div>
                          <div className="text-[10px] text-concrete-400">assessed</div>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {mode === "address" && (
              <div>
                <label className="block text-xs font-medium text-concrete-500 mb-1.5">
                  Street Address
                  <span className="ml-2 text-[10px] text-teal-500 font-normal">Powered by Google</span>
                </label>
                <div className="relative">
                  <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-concrete-400 z-10 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  ...
                  <input
                    ...
                    className="google-pac-input w-full pl-10 pr-4 py-2.5 rounded-none border border-concrete-200 bg-white text-concrete-900 text-sm placeholder:text-concrete-300 focus:outline-none focus:border-signal focus:ring-1 focus:ring-signal/20 transition"
                  />
                </div>
                {selectedCoords && (
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-none bg-signal-bg border border-signal/30 text-[11px] text-signal">
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                        Address found
                      </span>
                      <span className="text-[10px] text-concrete-400">{selectedCoords.lat.toFixed(5)}, {selectedCoords.lng.toFixed(5)}</span>
                    </div>

                    {/* Strata building: prompt for unit number */}
                    {isStrataBuilding && !selectedMatch && (
                      <div className="p-3 rounded-none border border-signal/30 bg-signal-bg/50">
                        ...
                        <div className="relative">
                          <input
                            ...
                            autoComplete="off"
                            className="w-full px-3 py-2 rounded-none border border-signal bg-white text-concrete-900 text-sm placeholder:text-concrete-300 focus:outline-none focus:border-signal focus:ring-1 focus:ring-signal/20 transition"
                          />
                          {showUnitDropdown && unitSuggestions.length > 0 && (
                            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-concrete-200 rounded-none shadow-none overflow-hidden max-h-[200px] overflow-y-auto">
                              {unitSuggestions.map((u) => (
                                <button
                                  ...
                                  className="w-full text-left px-3 py-2 text-xs hover:bg-signal-bg transition flex items-center justify-between border-t border-concrete-100 first:border-t-0"
                                >
                                  <span className="font-medium">Unit {u.unit_number}</span>
                                  <span className="text-concrete-500">{formatCurrency(u.assessed_value)}</span>
                                </button>
                              ))}
                            </div>
                          )}
                        ...
                        <div className="space-y-1.5">
                          {duplexUnits.map((u) => (
                            <button
                              ...
                              className="w-full text-left px-3 py-2 rounded-none border border-concrete-200 bg-white hover:border-signal hover:bg-signal-bg/30 text-concrete-700 text-xs transition"
                            >
                              <div className="flex items-center justify-between">
                                <div>
                                  <span className="font-medium">Unit {u.unit_number}</span>
                                  <span className="ml-2 text-concrete-400">PID: {u.pid}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                  <span className="px-1.5 py-0.5 rounded bg-signal-bg border border-signal/30 text-[10px] font-medium uppercase">
                                    Duplex
                                  </span>
                    ...
                    {addressMatches.length > 1 && (
                      <div className="p-3 rounded-none border border-amber-200 bg-amber-50/50">
                        ...
                        <div className="space-y-1.5">
                          {addressMatches.map((m) => (
                            <button
                              ...
                              className={`w-full text-left px-3 py-2 rounded-none border text-xs transition ${
                                selectedMatch?.pid === m.pid
                                  ? "border-signal bg-signal-bg text-teal-800"
                                  : "border-concrete-200 bg-white hover:border-signal hover:bg-signal-bg/30 text-concrete-700"
                              }`}
                            >
                              <div className="flex items-center justify-between">
                                <div>
                                  <span className="font-medium">{m.address}</span>
                                  <span className="ml-2 text-concrete-400">PID: {m.pid}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                  <span className="px-1.5 py-0.5 rounded bg-concrete-100 text-[10px] font-medium uppercase">
                                    {m.property_type}
                                  </span>
                                  <span className="text-concrete-500">
                    ...
                    {selectedMatch && addressMatches.length === 0 && (
                      <div className="flex items-center gap-2 text-[11px] text-signal">
                        ...
                        {selectedMatch.assessed_value > 0 && (
                          <span className="text-concrete-500">{formatCurrency(selectedMatch.assessed_value)}</span>
                        )}
                        <span className="text-concrete-400">PID: {selectedMatch.pid}</span>
                      </div>
                    )}
                  </div>
                )}
            ...
            {mode === "coordinates" && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-concrete-500 mb-1.5">Latitude</label>
                  <input type="text" value={lat} onChange={(e) => setLat(e.target.value)} placeholder="49.2827"
                    className="w-full px-4 py-2.5 rounded-none border border-concrete-200 bg-white text-concrete-900 text-sm placeholder:text-concrete-300 focus:outline-none focus:border-signal focus:ring-1 focus:ring-signal/20 transition" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-concrete-500 mb-1.5">Longitude</label>
          ...
          <div className="w-full sm:w-[200px] shrink-0">
            <label className="block text-xs font-medium text-concrete-500 mb-1.5">Property Type</label>
            <select value={propertyType} onChange={(e) => setPropertyType(e.target.value)}
              className="w-full px-4 py-2.5 rounded-none border border-concrete-200 bg-white text-concrete-900 text-sm focus:outline-none focus:border-signal focus:ring-1 focus:ring-signal/20 transition">
          ...
          <button type="submit" disabled={loading}
            className="w-full sm:w-auto shrink-0 px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-signal to-signal-dark rounded-none hover:from-signal-dark hover:to-signal-dark transition shadow-none disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? (
      ...
      {error && (
        <div className="card-hairline p-4 border-rose-200 bg-neg/10">
    ...
    <div className="space-y-6 animate-fade-in-up" style={{ animationFillMode: "forwards" }}>
      ...
      <div className="card-hairline p-8">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="text-xs font-medium text-concrete-400 uppercase tracking-wider mb-2">
              Estimated Market Value
            </div>
            <div className="hero-number text-3xl sm:text-4xl">{formatCurrencyFull(primaryEstimate)}</div>
            ...
            {marketEstimate && (
              <div className="mt-4 text-xs text-concrete-500 leading-relaxed max-w-xl">
          ...
          <div className="text-right">
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-none border ${gradeBg(result.confidence_grade)}`}>
              ...
              <span className="text-xs text-concrete-500">Confidence<br />Grade</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 relative">
          <div className="h-2 bg-concrete-100 rounded-none overflow-hidden">
            <div
              className="h-full rounded-none bg-gradient-to-r from-teal-400 to-teal-600 transition-all duration-1000"
              style={{
          ...
          <div className="flex justify-between mt-1.5 text-[10px] text-concrete-400">
      ...
      <div className="flex gap-0 border-2 border-concrete-900 w-fit">
        {[
          { key: "overview" as const, label: "BREAKDOWN" },
          { key: "cma" as const, label: `CMA${cma ? ` (${cma.comparable_count})` : ""}` },
          { key: "details" as const, label: "DETAILS" },
        ].map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`px-5 py-2 text-[10px] font-bold tracking-[0.15em] transition border-r-2 border-concrete-900 last:border-r-0 ${
              activeTab === tab.key ? "bg-concrete-900 text-white" : "bg-white text-concrete-500 hover:bg-concrete-100"
            }`}
          >
            {tab.label}
          </button>
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="card-hairline p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-concrete-700 uppercase tracking-wider">BC Assessment</h4>
              </div>
              <div className="text-2xl font-semibold text-concrete-900" style={{ fontFamily: "var(--font-display)" }}>
            ...
            {/* SAR Market Estimate */}
            <div className="card-hairline p-5 border-signal/30 bg-signal-bg/30">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-2 h-2 rounded-none bg-signal-bg0" />
                <h4 className="text-xs font-semibold text-concrete-700 uppercase tracking-wider">Market Value (SAR)</h4>
              </div>
              <div className="text-2xl font-semibold text-concrete-900" style={{ fontFamily: "var(--font-display)" }}>
            ...
            {/* CMA Estimate */}
            <div className={`card-hairline p-5 ${cma?.cma_estimate ? "border-emerald-200 bg-pos/10/30" : ""}`}>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-2 h-2 rounded-none bg-pos/100" />
                <h4 className="text-xs font-semibold text-concrete-700 uppercase tracking-wider">CMA Estimate</h4>
              </div>
              <div className="text-2xl font-semibold text-concrete-900" style={{ fontFamily: "var(--font-display)" }}>
          ...
          <div className="card-hairline p-6">
            <h3 className="text-sm font-semibold text-concrete-800 mb-5">How We Arrived at This Estimate</h3>
            <div className="space-y-4">
              {/* Step 1 */}
              <div className="flex gap-4">
                <div className="flex flex-col items-center">
                  <div className="w-7 h-7 rounded-none bg-sky-100 text-sky-700 flex items-center justify-center text-xs font-bold">1</div>
                  <div className="w-px flex-1 bg-sand-200 mt-1" />
                </div>
                <div className="pb-4">
                  <div className="text-sm font-medium text-concrete-800">BC Assessment Base Value</div>
                  ...
                  <div className="mt-2 text-sm font-medium text-concrete-700">
                    {assessedValue ? formatCurrencyFull(assessedValue) : "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <div className="flex flex-col items-center">
                  <div className="w-7 h-7 rounded-none bg-teal-100 text-signal flex items-center justify-center text-xs font-bold">2</div>
                  <div className="w-px flex-1 bg-sand-200 mt-1" />
                </div>
                <div className="pb-4">
                  <div className="text-sm font-medium text-concrete-800">Sub-Region Market Performance (SAR)</div>
                  <p className="text-xs text-concrete-500 mt-1">
                    Analyzed how properties in the <span className="font-medium text-concrete-700">{result.market_context.neighbourhood_name}</span> sub-region
                    actually sell compared to their assessed values.
                    {sarValue && (
                      <> Properties here have been selling at <span className="font-medium text-concrete-700">{(sarValue * 100).toFixed(1)}%</span> of their assessed values.</>
                    )}{" "}
                    Multiplying the assessed value by this ratio gives us the market-derived estimate.
                  </p>
                  <div className="mt-2 flex items-center gap-3">
                    <span className="text-sm font-medium text-concrete-700">{formatCurrencyFull(marketEstimate)}</span>
                    {sarValue && assessedValue && (
                      <span className={`text-xs px-2 py-0.5 rounded-none ${sarValue >= 1 ? "bg-pos/10 text-pos" : "bg-neg/10 text-neg"}`}>
              ...
              {result.adjustments.length > 0 && (
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="w-7 h-7 rounded-none bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold">{marketEstimate ? "3" : "2"}</div>
                    <div className="w-px flex-1 bg-sand-200 mt-1" />
                  </div>
                  <div className="pb-4">
                    <div className="text-sm font-medium text-concrete-800">Property-Specific Adjustments</div>
                    <p className="text-xs text-concrete-500 mt-1 mb-3">
                      Applied adjustments for property-specific factors that affect value beyond what the base model captures.
                    </p>
                    <div className="space-y-2">
                      {result.adjustments.map((adj, i) => (
                        <div key={i} className="flex items-center justify-between py-1.5 px-3 rounded-none bg-concrete-50">
                          <div>
                            <span className="text-sm text-concrete-800">{adj.name}</span>
                            <span className="text-xs text-concrete-400 ml-2">{adj.explanation}</span>
                          </div>
                          <div className={`text-sm font-medium ${adj.percentage >= 0 ? "text-pos" : "text-neg"}`}>
                            {adj.percentage >= 0 ? "+" : ""}{adj.percentage.toFixed(1)}%
                            <span className="text-xs text-concrete-400 ml-1">
              ...
              {cma && cma.comparable_count > 0 && (
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="w-7 h-7 rounded-none bg-emerald-100 text-pos flex items-center justify-center text-xs font-bold">
                      {result.adjustments.length > 0 ? (marketEstimate ? "4" : "3") : (marketEstimate ? "3" : "2")}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-concrete-800">CMA Validation</div>
                    <p className="text-xs text-concrete-500 mt-1">
                    ...
                    {cma.recommendation?.estimated_range && (
                      <div className="mt-2 text-xs text-concrete-500">
                        Blended recommendation: <span className="font-medium text-concrete-700">{formatCurrencyFull(cma.recommendation.estimated_range.low)}</span>
                        {" "}&ndash;{" "}
                        <span className="font-medium text-concrete-700">{formatCurrencyFull(cma.recommendation.estimated_range.high)}</span>
                        <span className="ml-2 text-concrete-400">({cma.recommendation.method})</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {/* Market Context */}
          <div className="card-hairline p-6">
            <h3 className="text-sm font-semibold text-concrete-800 mb-4">Neighbourhood Market Context</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-6">
              {[
                ...
                <div key={item.label}>
                  <div className="text-[11px] text-concrete-400 uppercase tracking-wider">{item.label}</div>
                  <div className="text-sm font-medium text-concrete-800 mt-1">{item.value}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === "details" && (
        <div className="space-y-6">
          ...
          <div className="card-hairline p-6">
            <h3 className="text-sm font-semibold text-concrete-800 mb-4">
              Assessment-Based Comparables ({result.comparables.length})
            </h3>
            <p className="text-xs text-concrete-400 mb-4">
            ...
            {result.comparables.length > 0 ? (
                  ...
                  <thead>
                    <tr className="border-b border-concrete-200">
                      {["Address", "Assessed Value", "Distance", "Similarity", "Year Built", "Zoning"].map((h) => (
                        <th key={h} className="text-left py-2 px-3 text-[11px] font-medium text-concrete-400 uppercase tracking-wider">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {result.comparables.map((comp, i) => (
                      <tr key={i} className="border-b border-concrete-100 last:border-0 hover:bg-concrete-50 transition">
                        <td className="py-2.5 px-3 text-concrete-800">{comp.address}</td>
                        <td className="py-2.5 px-3 text-concrete-700">{formatCurrencyFull(comp.assessed_value)}</td>
                        <td className="py-2.5 px-3 text-concrete-500">{formatDistance(comp.distance_m)}</td>
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1.5 bg-concrete-100 rounded-none overflow-hidden">
                              <div className="h-full bg-signal-bg0 rounded-none" style={{ width: `${Math.max(5, (1 - comp.similarity_score) * 100)}%` }} />
                            </div>
                            <span className="text-[11px] text-concrete-400">{((1 - comp.similarity_score) * 100).toFixed(0)}%</span>
                          </div>
                        </td>
                        <td className="py-2.5 px-3 text-concrete-500">{comp.year_built || "\u2014"}</td>
                        <td className="py-2.5 px-3 text-concrete-500">{comp.zoning || "\u2014"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="text-sm text-concrete-400">No comparable properties found</p>
            )}
          </div>
          ...
          {result.risk_flags.length > 0 && (
            <div className="card-hairline p-6">
              <h3 className="text-sm font-semibold text-concrete-800 mb-4">Risk Flags</h3>
              <div className="space-y-3">
                {result.risk_flags.map((flag, i) => (
                  <div key={i} className={`flex items-start gap-3 p-3 rounded-none border ${severityColor(flag.severity)}`}>
          ...
          {/* Metadata */}
          <div className="flex items-center justify-between text-[11px] text-concrete-400 px-1">
...
function CMAReportSection({ cma }: { cma: CMAResponse | null }) {
  if (!cma) {
    return (
      <div className="card-hairline p-8 text-center">
        <p className="text-sm text-concrete-400">CMA data not available for this property.</p>
      </div>
    );
  }
  ...
  if (cma.comparable_count === 0) {
    return (
      <div className="card-hairline p-8 text-center">
        <h3 className="text-sm font-semibold text-concrete-800 mb-2">Comparative Market Analysis</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="card-hairline p-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div>
            <h3 className="text-lg font-semibold text-concrete-900" style={{ fontFamily: "var(--font-display)" }}>
          ...
          {cma.recommendation?.estimated_value && (
            <div className="text-right">
              <div className="text-xs text-concrete-400 uppercase tracking-wider">Recommended Value</div>
              <div className="text-xl font-semibold text-signal mt-1" style={{ fontFamily: "var(--font-display)" }}>
                {formatCurrencyFull(cma.recommendation.estimated_value)}
              </div>
              {cma.recommendation.estimated_range && (
                <div className="text-xs text-concrete-400 mt-0.5">
        ...
        <div className="bg-concrete-50 rounded-none p-4 mb-6">
          <div className="text-[11px] text-concrete-400 uppercase tracking-wider mb-2">Subject Property</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            <div>
              <div className="text-[10px] text-concrete-400">Address</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5">{cma.subject.address || "N/A"}</div>
            </div>
            <div>
              <div className="text-[10px] text-concrete-400">Type</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5 capitalize">{cma.subject.property_type || "N/A"}</div>
            </div>
            <div>
              <div className="text-[10px] text-concrete-400">Beds/Baths</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5">
                {cma.subject.bedrooms ?? "\u2014"} / {cma.subject.bathrooms ?? "\u2014"}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-concrete-400">Floor Area</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5">
                {cma.subject.floor_area ? `${Math.round(cma.subject.floor_area).toLocaleString()} sqft` : "\u2014"}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-concrete-400">Year Built</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5">{cma.subject.year_built ?? "\u2014"}</div>
            </div>
            <div>
              <div className="text-[10px] text-concrete-400">Assessed Value</div>
              <div className="text-sm font-medium text-concrete-800 mt-0.5">
      ...
      <div className="card-hairline overflow-hidden">
        <div className="p-4 border-b border-concrete-200 bg-concrete-50/50">
          <h4 className="text-sm font-semibold text-concrete-800">Comparable Sold Properties</h4>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-concrete-50 border-b border-concrete-200">
                {["#", "Address", "Type", "Sold Price", "List Price", "SP/LP", "Sold Date", "DOM", "Bed", "Bath", "Sqft", "Year", "Dist", "Adj. Price"].map((h) => (
                  <th key={h} className="text-left py-2.5 px-3 text-[10px] font-semibold text-concrete-500 uppercase tracking-wider whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
      ...
      <div className="card-hairline p-6">
        <h4 className="text-sm font-semibold text-concrete-800 mb-4">Price Adjustments Detail</h4>
        <p className="text-xs text-concrete-400 mb-4">
        ...
        <div className="space-y-3">
          {comps.map((comp, i) => (
            <div key={comp.mls_number} className="border border-concrete-100 rounded-none p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-none bg-concrete-100 text-sand-600 flex items-center justify-center text-[10px] font-bold">{i + 1}</span>
                  <span className="text-sm font-medium text-concrete-800">{comp.address}</span>
                  <span className="text-xs text-concrete-400 font-mono">MLS# {comp.mls_number}</span>
                </div>
                <div className="text-right">
                  <span className="text-xs text-concrete-400">Sold: </span>
                  <span className="text-sm font-medium text-concrete-800">{formatCurrencyFull(comp.sold_price)}</span>
                  <span className="mx-2 text-concrete-300">&rarr;</span>
                  <span className="text-sm font-semibold text-signal">{formatCurrencyFull(comp.adjusted_price)}</span>
                </div>
              </div>
              {comp.adjustments.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2">
                  {comp.adjustments.map((adj, j) => (
                    <div key={j} className="flex items-center justify-between py-1 px-3 rounded bg-concrete-50 text-xs">
                      <span className="text-sand-600">{adj.name}: {adj.detail}</span>
                      <span className={`font-medium ${adj.dollar >= 0 ? "text-pos" : "text-neg"}`}>
                ...
                <div className="text-xs text-concrete-400 mt-1">No adjustments needed</div>
              )}
            </div>
          ))}
        </div>
      ...
      {cma.cma_range && (
        <div className="card-hairline p-6">
          <h4 className="text-sm font-semibold text-concrete-800 mb-4">CMA Price Range Summary</h4>
          <div className="grid grid-cols-4 gap-4 mb-4">
            <div className="text-center p-3 rounded-none bg-concrete-50">
              <div className="text-[10px] text-concrete-400 uppercase">Low (25th %ile)</div>
              <div className="text-lg font-semibold text-concrete-700 mt-1">{formatCurrency(cma.cma_range.low)}</div>
            </div>
            <div className="text-center p-3 rounded-none bg-signal-bg border border-signal/30">
              <div className="text-[10px] text-signal uppercase font-medium">Median</div>
              <div className="text-lg font-semibold text-signal mt-1">{formatCurrency(cma.cma_range.median)}</div>
            </div>
            <div className="text-center p-3 rounded-none bg-concrete-50">
              <div className="text-[10px] text-concrete-400 uppercase">Mean</div>
              <div className="text-lg font-semibold text-concrete-700 mt-1">{formatCurrency(cma.cma_range.mean)}</div>
            </div>
            <div className="text-center p-3 rounded-none bg-concrete-50">
              <div className="text-[10px] text-concrete-400 uppercase">High (75th %ile)</div>
              <div className="text-lg font-semibold text-concrete-700 mt-1">{formatCurrency(cma.cma_range.high)}</div>
            </div>
          </div>

          {/* Visual range bar */}
          <div className="relative h-3 bg-concrete-100 rounded-none overflow-hidden mt-4">
            <div
              className="absolute h-full bg-gradient-to-r from-teal-300 to-teal-500 rounded-none"
              style={{
          ...
          {cma.recommendation && cma.recommendation.estimated_value && (
            <div className="mt-6 p-4 rounded-none bg-signal-bg border border-signal/30">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs font-semibold text-signal uppercase tracking-wider">Final Recommendation</div>
                  <div className="text-sm text-signal mt-1">
                ...
                <div className="text-right">
                  ...
                  <div className={`inline-block mt-1 px-2 py-0.5 rounded-none text-[10px] font-semibold uppercase tracking-wider ${
                    cma.recommendation.confidence === "high" ? "bg-emerald-100 text-pos" :
                    cma.recommendation.confidence === "moderate" ? "bg-amber-100 text-amber-700" :
                    "bg-rose-100 text-neg"
                  }`}>
...
function CMARow({ comp, index }: { comp: CMAComparable; index: number }) {
  return (
    <tr className="border-b border-concrete-100 last:border-0 hover:bg-concrete-50/50 transition">
      <td className="py-2.5 px-3 text-concrete-400 text-xs">{index}</td>
      <td className="py-2.5 px-3">
        <div className="text-concrete-800 font-medium">{comp.address}</div>
        <div className="text-[10px] text-concrete-400 font-mono">MLS# {comp.mls_number}</div>
      </td>
      <td className="py-2.5 px-3 text-concrete-500 text-xs">{comp.property_type}</td>
      <td className="py-2.5 px-3 font-medium text-concrete-800">{formatCurrencyFull(comp.sold_price)}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.list_price ? formatCurrencyFull(comp.list_price) : "\u2014"}</td>
      <td className="py-2.5 px-3">
        {comp.list_to_sold ? (
          <span className={`text-xs font-medium ${comp.list_to_sold >= 1 ? "text-pos" : "text-neg"}`}>
            {(comp.list_to_sold * 100).toFixed(1)}%
          </span>
        ) : "\u2014"}
      </td>
      <td className="py-2.5 px-3 text-concrete-500 text-xs whitespace-nowrap">{comp.sold_date}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.dom ?? "\u2014"}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.bedrooms ?? "\u2014"}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.bathrooms ?? "\u2014"}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.floor_area ? comp.floor_area.toLocaleString() : "\u2014"}</td>
      <td className="py-2.5 px-3 text-concrete-500">{comp.year_built ?? "\u2014"}</td>
      <td className="py-2.5 px-3 text-concrete-500 text-xs">{formatDistance(comp.distance_m)}</td>
      <td className="py-2.5 px-3 font-medium text-signal">{formatCurrencyFull(comp.adjusted_price)}</td>
    </tr>
  );
}
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="text-center p-3 rounded-none bg-concrete-50">
      <div className="text-[10px] text-concrete-400 uppercase tracking-wider">{label}</div>
      <div className="text-sm font-semibold text-concrete-800 mt-1">{value}</div>
      {sub && <div className="text-[10px] text-concrete-400">{sub}</div>}
    </div>
  );
}
